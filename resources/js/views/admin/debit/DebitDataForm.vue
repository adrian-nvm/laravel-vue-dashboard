  ...
  <div>
    ...
    <ul class="nav nav-tabs" id="debitDataTabs" role="tablist">
      <li class="nav-item" role="presentation">
        <button class="nav-link active" id="line-tab" data-toggle="tab" data-target="#line" type="button" role="tab" aria-controls="line" aria-selected="true">Debit Line</button>
      </li>
      <li class="nav-item" role="presentation">
        <button class="nav-link" id="hana-tab" data-toggle="tab" data-target="#hana" type="button" role="tab" aria-controls="hana" aria-selected="false">Debit MyHana</button>
      </li>
    </ul>
              ...
              <div class="card-body">
                <form @submit.prevent="submitLineForm">
                  ...
                  <div class="form-group">
                    ...
                    <input type="number" class="form-control" id="debitTrxFreq" v-model.number="lineForm.debitTrxFreq">
                  </div>
                  <div class="form-group">
                    <label for="debitTrxAmt">Debit Transaction Amount</label>
                    <input type="number" class="form-control" id="debitTrxAmt" v-model.number="lineForm.debitTrxAmt">
                  </div>
                  <div class="form-group">
                    <label for="debitUniqueCifQty">Debit Unique CIF Quantity</label>
                    <input type="number" class="form-control" id="debitUniqueCifQty" v-model.number="lineForm.debitUniqueCifQty">
                  </div>
      ...
      <div class="tab-pane fade" id="hana" role="tabpanel" aria-labelledby="hana-tab">
        <div class="row mt-4">
          <div class="col-lg-12">
            <div class="card shadow mb-4">
              <div class="card-header py-3">
                <h6 class="m-0 font-weight-bold text-primary">Input Debit MyHana Data</h6>
              </div>
              <div class="card-body">
                <form @submit.prevent="submitHanaForm">
                  ...
                  <div class="form-group">
                    <label for="debitMyhanaTrxFreq">Debit MyHana Transaction Frequency</label>
                    <input type="number" class="form-control" id="debitMyhanaTrxFreq" v-model.number="hanaForm.debitMyhanaTrxFreq">
                  </div>
                  <div class="form-group">
                    <label for="debitMyhanaTrxAmt">Debit MyHana Transaction Amount</label>
                    <input type="number" class="form-control" id="debitMyhanaTrxAmt" v-model.number="hanaForm.debitMyhanaTrxAmt">
                  </div>
                  <div class="form-group">
                    <label for="debitMyhanaUniqueCifQty">Debit MyHana Unique CIF Quantity</label>
                    <input type="number" class="form-control" id="debitMyhanaUniqueCifQty" v-model.number="hanaForm.debitMyhanaUniqueCifQty">
                  </div>
          ...
          <div class="col-lg-12">
            <div class="card shadow mb-4">
              <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                <h6 class="m-0 font-weight-bold text-primary">Debit MyHana Data</h6>
    ...
    return {
      ...
      hanaColumns: [
        ...
        },
        {
          ...
          field: 'debitMyhanaTrxFreq',
          tdClass: 'text-right',
          formatFn: this.formatCurrency,
        },
        {
          label: 'Transaction Amount',
          field: 'debitMyhanaTrxAmt',
          tdClass: 'text-right',
          formatFn: this.formatCurrency,
        },
        {
          label: 'Unique CIF Quantity',
          field: 'debitMyhanaUniqueCifQty',
          tdClass: 'text-right',
          formatFn: this.formatCurrency,
        },
      ],
  ...
  methods: {
    ...
    async fetchHanaData() {
      ...
      try {
        ...
        let url = `/debit/hana-data?page=${page}&per_page=${per_page}&start_dt=${start_dt}`;
    ...
    async submitLineForm() {
      try {
        ...
        const response = await axios.post('debit/line-data', submissionData);
    ...
    async submitHanaForm() {
      try {
        ...
        const response = await axios.post('debit/hana-data', submissionData);
        this.fetchHanaData();
        this.toast.success(response.data.message);
      } catch (error) {
        handleError(error, this.toast);
    ...
    async exportHanaData() {
      try {
        const response = await axios.get('/debit/hana-data?per_page=-1');
        const csv = Papa.unparse(response.data.data);
        const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
        const link = document.createElement('a');
        link.href = URL.createObjectURL(blob);
        link.setAttribute('download', 'debit-hana-data.csv');
        document.body.appendChild(link);
        link.click();
        document.body.removeChild(link);
      } catch (error) {
