  ...
  methods: {
    ...
    async fetchLineData() {
      ...
      try {
        ...
        let url = `/biller/line-data?page=${page}&per_page=${per_page}&start_dt=${start_dt}&start_month=${start_month}&end_month=${end_month}`;
    ...
    async exportLineData() {
      try {
        const response = await axios.get('/biller/line-data?per_page=-1');
